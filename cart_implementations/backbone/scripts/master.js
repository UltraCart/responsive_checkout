...
app.commonFunctions.checkout = function (paymentMethod) {
    ...
    if (!app.data.bootstrap.get('processingOrder')) {
        ...
        jQuery.ajax({
            url: 'https://api.ultracart.com/rest/cart/checkout',
            type: 'POST', // Notice
...
app.commonFunctions.estimateShipping = function () {
    ...
    if (!app.data.bootstrap.get('fetchingShipping')) {
        ...
        jQuery.ajax({
            url: 'https://api.ultracart.com/rest/cart',
            type: 'PUT', // Notice
            headers: {"cache-control": "no-cache"},
            contentType: 'application/json; charset=UTF-8',
            ...
            jQuery.ajax({
                url: 'https://api.ultracart.com/rest/cart/estimateShipping',
                headers: {"cache-control": "no-cache"},
                type: 'POST',
                async: true,
                'contentType': 'application/json; charset=UTF-8',
...
app.models.Cart = uc.models.DeepAndNestedModel.extend({
    ...
    idAttribute: "cartId",
    url: 'https://api.ultracart.com/rest/cart' // TODO verify this works.
});
...
app.views.Credentials = Backbone.View.extend({
    ...
    'submitLogout': function () {
        ...
        var restWrapper = new ultracart.Cart(window.merchantId); // both variables are defined in cart.html
...
app.views.Items = Backbone.View.extend({
    ...
    render: function () {
        ...
        this.collection.each(function (model) {
            ...
            that.childViews.push(new app.views.Item({
                model: model,
                position: position,
                totalItems: that.collection.length
            }));
            position++;
        });
